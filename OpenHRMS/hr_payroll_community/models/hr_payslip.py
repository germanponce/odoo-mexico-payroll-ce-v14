...
from odoo.addons.hr_payroll_community.models.browsable_object import BrowsableObject, InputLine, WorkedDays, Payslips
        ...
        if self.env.context.get('payslip_generate_pdf'):
            for payslip in self:
                if not payslip.struct_id or not payslip.struct_id.report_id:
                    report = self.env.ref('hr_payroll_community.action_report_payslip', False)
                else:
    ...
    def _get_worked_day_lines(self, domain=None, check_out_of_contract=True):
        ...
        if contract.resource_calendar_id:
            ...
            if out_days or out_hours:
                work_entry_type = self.env.ref('hr_payroll_community.hr_work_entry_type_out_of_contract')
                res.append({
                    'sequence': work_entry_type.sequence,
