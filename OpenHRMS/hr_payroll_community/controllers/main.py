...
class HrPayroll(Controller):
    ...
    def get_payroll_report_print(self, list_ids='', **post):
        if not request.env.user.has_group('hr_payroll_community.group_hr_payroll_user') or not list_ids or re.search("[^0-9|,]", list_ids):
        ...
        for payslip in payslips:
            if not payslip.struct_id or not payslip.struct_id.report_id:
                report = request.env.ref('hr_payroll_community.action_report_payslip', False)
            else:
